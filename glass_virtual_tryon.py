    ...
    if ret:
        ...
        if len(faces) > 0:
            ...
            for (face_x, face_y, face_w, face_h) in faces:
                ...
                # Get the center of the 2 eyes
                for (eye_x, eye_y, eye_w, eye_h) in eyes:
                    eye_centers.append((face_x + int(eye_x + eye_w/2), face_y + int(eye_y + eye_h/2)))

                if len(eye_centers) >=2:

                    # 3. Calculate the coordinates and size of the glasses
                    ...
                    # Calculate the coordinates of the glasses
                    ...
                    # 4. Draw the glasses onto the face
                    ...
                    # Extract the background and face (excluding the glasses) from the original image
                    background = cv2.bitwise_and(image, image, mask = mask)

                    mask_inv = cv2.bitwise_not(mask)

                    # Extract the glasses portion from the overlay
